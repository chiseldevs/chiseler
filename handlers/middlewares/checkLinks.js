...
const dh = {
	blacklistedDomain: R.always(Action.Warn('Quyidagi ssilka qora ro\'yxatdan')),
	...
	tme: async url => {
		...
		if (url.searchParams.has('start')) return Action.Warn('Botni sindirishga harakat');
		...
		return Action.Warn('Telegram guruh va kanalga reklama');
	},
};
...
const classifyAsync = memoize(async url => {
	...
	if (blacklisted.protocol(url)) return Action.Warn('Ssilka telegram protocolini ishlatayabdi');
...
module.exports = async (ctx, next) =>
	(await classifyCtx(ctx)).cata({
		...
		Notify(error) {
			...
			if (notifyBrokenLink) {
				ctx.reply(
					`️ℹ️ Quyidagi ${error.url} ssilka (${error.code}) xatoligi ostida ishlamayabdi.`,
					{ reply_to_message_id }
				);
			}
			return next();
