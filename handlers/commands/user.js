...
const getWarnsHandler = async ({ from, message, reply }) => {
	if (!from) {
		return reply(
			'ℹ️ <b>Bu komanda kanallarda ishlatish taqiqlanadi.</b>',
			replyOptions
	...
	if (targets.length > 1) {
		return reply(
			'ℹ️ <b>Bironta foydalanuvchini ko\'rsating.</b>',
			replyOptions
		).then(scheduleDeletion());
	}
